...
use vector_lib::{
    codecs::encoding::{BatchSerializerConfig, FramingConfig, SerializerConfig},
...
use crate::{
    codecs::{EncodingConfigWithFraming, TimestampFormat, Transformer},
...
async fn insert_events_unix_timestamps() {
    ...
    let config = ClickhouseConfig {
        ...
        table: table.clone().try_into().unwrap(),
        compression: Compression::None,
        encoding: Some(EncodingConfigWithFraming::new(
            Some(FramingConfig::NewlineDelimited),
            SerializerConfig::Json(Default::default()),
            Transformer::new(None, None, Some(TimestampFormat::Unix)).unwrap(),
        )),
        batch,
        request: TowerRequestConfig {
            retry_attempts: 1,
