...
fn clickhouse_type_to_arrow(ch_type: &str) -> Result<(DataType, bool), String> {
    ...
    let data_type = match base_type {
        ...
        // Complex types
        _ if base_type.starts_with("Array") => parse_array_type(base_type)?,
        _ if base_type.starts_with("Tuple") => parse_tuple_type(base_type)?,
        _ if base_type.starts_with("Map") => {
            return Err(format!(
                "Map type '{}' is not supported. ClickHouse Map types cannot be automatically converted to Arrow format.",
                ch_type
            ));
        }
...
/// Parses ClickHouse Array types and returns the appropriate Arrow List type.
/// Example: Array(Int32) -> DataType::List(Field::new("item", DataType::Int32, true))
/// Supports nested arrays: Array(Array(String))
fn parse_array_type(ch_type: &str) -> Result<DataType, String> {
    ...
    let args = match parse_args(args_str) {
        ...
        Ok(_) => {
            warn!("Array type expects exactly 1 argument, got '{}'", ch_type);
            return Ok(DataType::Utf8);
        }
        Err(_) => {
            warn!("Could not parse Array type arguments from '{}'", ch_type);
            return Ok(DataType::Utf8);
        }
    };

    // Recursively parse the element type (no metadata for nested types)
    let (element_type, element_nullable) = clickhouse_type_to_arrow(&args[0])?;

    // Arrow List fields are always nullable in ClickHouse
    let field = Field::new("item", element_type, element_nullable);
    Ok(DataType::List(Arc::new(field)))
}

/// Parses ClickHouse Tuple types and returns the appropriate Arrow Struct type.
/// Supports both named and unnamed tuples:
/// - Named: Tuple(host String, status Int32)
/// - Unnamed: Tuple(Int32, String, Float64)
fn parse_tuple_type(ch_type: &str) -> Result<DataType, String> {
    ...
    let args = match parse_args(args_str) {
        ...
        Ok(_) => {
            warn!("Tuple type expects at least 1 argument, got '{}'", ch_type);
            return Ok(DataType::Utf8);
        }
        Err(_) => {
            warn!("Could not parse Tuple type arguments from '{}'", ch_type);
            return Ok(DataType::Utf8);
        }
    };
    ...
    for (idx, arg) in args.iter().enumerate() {
        ...
        let (field_name, field_type_str) = if let Some(space_pos) = arg.find(char::is_whitespace) {
            // Named field: "field_name Type"
            let name = arg[..space_pos].trim();
            let type_str = arg[space_pos..].trim();
            (name.to_string(), type_str)
        } else {
            // Unnamed field: use positional name
            (format!("field_{}", idx), arg.as_str())
        };

        // Recursively parse the field type (no metadata for nested types)
        let (field_type, field_nullable) =
            clickhouse_type_to_arrow(field_type_str)?;
        fields.push(Field::new(field_name, field_type, field_nullable));
    }
...
mod tests {
    ...
    fn test_clickhouse_type_mapping() {
        assert_eq!(convert_type_no_metadata("String").expect("Failed to convert ClickHouse type to Arrow"), (DataType::Utf8, false));
        assert_eq!(convert_type_no_metadata("Int64").expect("Failed to convert ClickHouse type to Arrow"), (DataType::Int64, false));
        assert_eq!(
            convert_type_no_metadata("Float64").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Float64, false)
        );
        assert_eq!(convert_type_no_metadata("Bool").expect("Failed to convert ClickHouse type to Arrow"), (DataType::Boolean, false));
        assert_eq!(
            convert_type_no_metadata("DateTime").expect("Failed to convert ClickHouse type to Arrow"),
    ...
    fn test_datetime64_precision_mapping() {
        assert_eq!(
            convert_type_no_metadata("DateTime64(0)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Second, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(3)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Millisecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(6)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Microsecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(9)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Nanosecond, None), false)
        );
        // Test with timezones
        assert_eq!(
            convert_type_no_metadata("DateTime64(9, 'UTC')").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Nanosecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(6, 'UTC')").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Microsecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(9, 'America/New_York')").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Nanosecond, None), false)
        );
        // Test edge cases for precision ranges
        assert_eq!(
            convert_type_no_metadata("DateTime64(1)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Millisecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(4)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Microsecond, None), false)
        );
        assert_eq!(
            convert_type_no_metadata("DateTime64(7)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Timestamp(TimeUnit::Nanosecond, None), false)
        );
    }
    ...
    fn test_nullable_type_mapping() {
        ...
        // Nullable types
        assert_eq!(
            convert_type_no_metadata("Nullable(String)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Utf8, true)
        );
        assert_eq!(
            convert_type_no_metadata("Nullable(Int64)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Int64, true)
        );
        assert_eq!(
            convert_type_no_metadata("Nullable(Float64)").expect("Failed to convert ClickHouse type to Arrow"),
    ...
    fn test_lowcardinality_type_mapping() {
        assert_eq!(
            convert_type_no_metadata("LowCardinality(String)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Utf8, false)
        );
        assert_eq!(
            convert_type_no_metadata("LowCardinality(FixedString(10))").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Utf8, false)
        );
        // Nullable + LowCardinality
        assert_eq!(
            convert_type_no_metadata("Nullable(LowCardinality(String))").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Utf8, true)
        );
    }
    ...
    fn test_decimal_type_mapping() {
        // Generic Decimal(P, S)
        assert_eq!(
            convert_type_no_metadata("Decimal(10, 2)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(10, 2), false)
        );
        assert_eq!(
            convert_type_no_metadata("Decimal(38, 6)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(38, 6), false)
        );
        assert_eq!(
            convert_type_no_metadata("Decimal(50, 10)").expect("Failed to convert ClickHouse type to Arrow"),
        ...
        assert_eq!(
            convert_type_no_metadata("Decimal( 18 , 6 )").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(18, 6), false)
        );

        // Decimal32(S) - precision up to 9
        assert_eq!(
            convert_type_no_metadata("Decimal32(2)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(9, 2), false)
        );
        assert_eq!(
            convert_type_no_metadata("Decimal32(4)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(9, 4), false)
        );

        // Decimal64(S) - precision up to 18
        assert_eq!(
            convert_type_no_metadata("Decimal64(4)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(18, 4), false)
        );
        assert_eq!(
            convert_type_no_metadata("Decimal64(8)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(18, 8), false)
        );

        // Decimal128(S) - precision up to 38
        assert_eq!(
            convert_type_no_metadata("Decimal128(10)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal128(38, 10), false)
        );

        // Decimal256(S) - precision up to 76
        assert_eq!(
            convert_type_no_metadata("Decimal256(20)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Decimal256(76, 20), false)
        );

        // With Nullable wrapper
        assert_eq!(
            convert_type_no_metadata("Nullable(Decimal(18, 6))").expect("Failed to convert ClickHouse type to Arrow"),
...
"#;
    ...
    fn test_array_type_mapping() {
        // Simple array types
        assert_eq!(
            convert_type_no_metadata("Array(Int32)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::List(Arc::new(Field::new("item", DataType::Int32, false))), false)
        );
        assert_eq!(
            convert_type_no_metadata("Array(String)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::List(Arc::new(Field::new("item", DataType::Utf8, false))), false)
        );
        assert_eq!(
            convert_type_no_metadata("Array(Float64)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::List(Arc::new(Field::new("item", DataType::Float64, false))), false)
        );

        // Nested arrays
        assert_eq!(
            convert_type_no_metadata("Array(Array(String))").expect("Failed to convert ClickHouse type to Arrow"),
        ...
        // Array with nullable element type
        assert_eq!(
            convert_type_no_metadata("Array(Nullable(Int32))").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::List(Arc::new(Field::new("item", DataType::Int32, true))), false)
        );

        // Array with LowCardinality element type
        assert_eq!(
            convert_type_no_metadata("Array(LowCardinality(String))").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::List(Arc::new(Field::new("item", DataType::Utf8, false))), false)
        );
    }
    ...
    fn test_tuple_type_mapping() {
        // Unnamed tuple
        assert_eq!(
            convert_type_no_metadata("Tuple(Int32, String, Float64)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Struct(
                vec![
                    Field::new("field_0", DataType::Int32, false),
                    Field::new("field_1", DataType::Utf8, false),
                    Field::new("field_2", DataType::Float64, false),
                ]
                .into()
            ), false)
        );
        ...
        // Named tuple
        assert_eq!(
            convert_type_no_metadata("Tuple(host String, status Int32)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Struct(
                vec![
        ...
        // Named tuple with LowCardinality
        assert_eq!(
            convert_type_no_metadata("Tuple(service LowCardinality(String), level String)").expect("Failed to convert ClickHouse type to Arrow"),
            (DataType::Struct(
                vec![
    ...
    fn test_complex_nested_types() {
        // Array of Tuples
        let result = convert_type_no_metadata("Array(Tuple(host String, count Int64))").expect("Failed to convert ClickHouse type to Arrow");
        match result.0 {
            DataType::List(field) => {
        ...
        // Array of Tuples with LowCardinality
        let result = convert_type_no_metadata(
            "Array(Tuple(service LowCardinality(String), duration Float64))",
