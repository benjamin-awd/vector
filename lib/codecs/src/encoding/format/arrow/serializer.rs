...
fn convert_timestamps(event: &LogEvent, schema: &SchemaRef) -> LogEvent {
    schema
        .fields()
        .iter()
        .fold(event.clone(), |mut acc, field| {
            if let DataType::Timestamp(unit, _) = field.data_type() {
                ...
                if let Some(Value::Timestamp(ts)) = event.get(lookup::event_path!(field_name))
                    && let Some(val) = timestamp_to_unit(ts, unit)
                {
                    acc.insert(field_name, Value::Integer(val));
                }
            }
            acc
        })
